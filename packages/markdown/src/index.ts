...
export function renderMarkdownLite(markdown: string): string {
  ...
  const inline = (s:string) => { let v = escapeHtml(s); v = v.replace(/`([^`]+)`/g,"<code>$1</code>").replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/\*([^*]+)\*/g,"<em>$1</em>").replace(/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g,'<a href="$2" target="_blank" rel="noreferrer">$1</a>').replace(/\[\[([^\]|]+)(?:\|([^\]]+))?\]\]/g,(_,t,l)=>`<span class="wikilink" data-target="${escapeHtml(t)}">${escapeHtml(l||t)}</span>`); return v; };
