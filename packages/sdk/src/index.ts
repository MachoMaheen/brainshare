...
export class BrainSharePublisherClient {
  ...
  async publishNote(id:string,path:string,markdown:string){const r=await this.f(this.url(`/api/notes/${id}`),{method:"PUT",headers:{...this.headers("text/markdown"),"x-note-path":path},body:markdown});return this.json(r,"publish note") as Promise<{url:string}>;}
